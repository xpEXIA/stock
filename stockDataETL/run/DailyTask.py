...
async def dailyTask(request, date: str):
    ...
    if get_trade_cal['is_open'].values[0] != 1:
        ...
        get_daily = get_TS_data.getDaily(trade_date=get_trade_cal['cal_date'].values[0])
        if get_daily is None:
            logger.error(f"ods_daily数据库数据导入失败, 数据为空, 交易日: {get_trade_cal['cal_date'].values[0]}")
            failure_list.append("ods_daily")
        data_load.append("ods_daily", get_daily)

        logger.info("开始获取每日指标数据, 表ods_daily_basic")
        get_daily_basic = get_TS_data.getDailyBasic(trade_date=get_trade_cal['cal_date'].values[0])
        if get_daily_basic is None:
            logger.error(f"ods_daily_basic数据库数据导入失败, 数据为空, 交易日: {get_trade_cal['cal_date'].values[0]}")
        ...
        for index_code_ in index_code:
            get_index_daily = get_TS_data.getIndexDaily(ts_code=index_code_,trade_date=get_trade_cal['cal_date'].values[0])
            if get_index_daily is None:
                logger.error(f"ods_index_daily数据库数据导入失败, 数据为空, 指数代码: {index_code_}, 交易日: {get_trade_cal['cal_date'].values[0]}")
                failure_list.append("ods_index_daily")
                continue
            data_load.append("ods_index_daily", get_index_daily)

        logger.info("开始获取资金流向数据, 表ods_moneyflow")
        get_moneyflow = get_TS_data.getMoneyFlow(trade_date=get_trade_cal['cal_date'].values[0])
        if get_moneyflow is None:
            logger.error(f"ods_moneyflow数据库数据导入失败, 数据为空, 交易日: {get_trade_cal['cal_date'].values[0]}")
            failure_list.append("ods_moneyflow")
        data_load.append("ods_moneyflow", get_moneyflow)

        logger.info("开始获取股票涨跌幅数据, 表ods_stk_limit")
        get_stk_limit = get_TS_data.getStkLimit(trade_date=get_trade_cal['cal_date'].values[0])
        if get_stk_limit is None:
            logger.error(f"ods_stk_limit数据库数据导入失败, 数据为空, 交易日: {get_trade_cal['cal_date'].values[0]}")
