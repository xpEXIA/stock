...
def initDatabase(table_list=None):
    ...
    if get_stock_basic.empty:
        logging.error("ods_stock_basic数据库数据导入失败, 数据为空")
    ...
    start_date = (datetime.today()
                .replace(month=datetime.strptime(datetime.today(), "%Y%m%d").month - 6)
                .strftime("%Y%m%d"))
    ...
    if get_trade_cal.empty:
        logging.error("ods_trade_cal数据库数据导入失败, 数据为空")
    ...
    if failure_list == []:
        logging.info("初始化数据库成功")
        return {"status": "success", "message": "初始化数据库成功"}
    return {"status": "error", "message": f"初始化数据库失败, 失败表: {failure_list}"}
